...
function Card({data,key}) {
  ...
  return (
    <Link to={`details/${data.id}`}>
        ...
        key={key} 
        className='flex flex-col gap-6 w-[16vmax] p-[1.5vmax] rounded-[7px] bg-[#12213e] cursor-pointer'>
            ...
            <div className='flex flex-col gap-2'>
                <div className=' justify-between'>
                    <p className='flex gap-[10px] font-semibold text-[1vmax]  text-white'>Price: <span className={`${data.price_change_percentage_24h < 0 ? "text-red-500" : "text-green-400"}`}>${data.current_price.toFixed(2)}</span>

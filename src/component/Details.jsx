...
const CoinDetails=({details})=>{
    ...
    return(
        <div className='py-10 overflow-hidden bg-[#040920] flex flex-col items-center justify-center gap-3'>
            <div>
                <label className='font-semibold text-[1.5vmax] text-white' for="type" >Currency: </label>
                <select className='px-[1.5max] py-[.5vmax] border-none font-medium bg-[#0f1d45] rounded-[5px] text-white' name="type" id="type" onClick={(e)=>setPrice(e.target.value.toLowerCase())}>
                    <option className='text-[1vmax] '>USD</option>
                    {
                        Object.keys(details.market_data.current_price).map((i)=><option className='text-[1vmax]'>{i.toUpperCase()}</option>)
                    }
                </select>
            </div>
            
            <div className='p-8 flex rounded-[20px] flex-col gap-4 bg-[#09132e] w-[30vw]'>
                <div className='flex item-center  gap-5'>
                    <img className="h-[7vmax]" src={details.image.large} />
                    <ul>
